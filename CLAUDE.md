...
Structure to know before touching it:
- `draw()` builds the **entire frame as one string** inside a single `$(...)` subshell and writes it in one `printf` (cursor-home + per-line clear-to-end-of-line, not a full screen clear) — this is what makes redraws flicker-free. Anything that must persist across redraws (`WAVE_FRAME`, the cached `STATION_BANNER_TEXT`, `SHOW_SETTINGS`/`SHOW_DANGER`) **must be mutated outside that subshell**, since subshell variable changes are lost when it exits.
- All dashboard content — status, live stelsel stats (buffer/data/CPU), and the collapsible INSTELLINGS/GEVAARLIK sections — lives on one continuously-redrawn main screen; there is no separate mode/screen to navigate into.
- Input is read one char at a time (`read_main_key`): a digit starts a full-line read (multi-digit settings menu numbers, terminated by Enter), any other character is dispatched instantly as a single-key command, a stray ESC is swallowed silently.
- `<20` terminal lines triggers `compact` mode, which only suppresses decorative section headers/spacing — it does not change which content is shown.
